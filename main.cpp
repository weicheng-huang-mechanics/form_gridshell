	...
	while ( myWorld.simulationRunning() > 0)
	{
		...
		*/
		...
		for (int n=0; n < NRODS; n++)
		{
			...
			{
				glVertex3f( myWorld.getScaledCoordinate(n, 4*i), myWorld.getScaledCoordinate(n, 4*i+1), myWorld.getScaledCoordinate(n, 4*i+2) );
				glVertex3f( myWorld.getScaledCoordinate(n, 4*(i+1)), myWorld.getScaledCoordinate(n, 4*(i+1)+1), myWorld.getScaledCoordinate(n, 4*(i+1)+2));
			}
			glEnd();
		}
